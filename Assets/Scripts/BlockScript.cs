...
{
    [Header("Sprites")] [SerializeField] private Sprite passableSprite = null;
    [SerializeField] private Sprite obstacleSprite = null;

    [Header("Text")] [SerializeField] private TextMesh northDistanceText = null;
    ...
    private SpriteRenderer jumpPointIndicator = null;
    ...
    [Header("Marker Colors")] [SerializeField]
    Color jumpPointDistanceColor = Color.blue;
    ...
    private void displayGreaterThanZeroJumpDistances()
    {
        ...
        northDistanceText.gameObject.SetActive(!nodeReference.isObstacle &&
                                               nodeReference.jpDistances[(int)eDirections.NORTH] > 0);
        northEastDistanceText.gameObject.SetActive(!nodeReference.isObstacle &&
                                                   nodeReference.jpDistances[(int)eDirections.NORTH_EAST] > 0);
        eastDistanceText.gameObject.SetActive(!nodeReference.isObstacle &&
                                              nodeReference.jpDistances[(int)eDirections.EAST] > 0);
        southEastDistanceText.gameObject.SetActive(!nodeReference.isObstacle &&
                                                   nodeReference.jpDistances[(int)eDirections.SOUTH_EAST] > 0);
        southDistanceText.gameObject.SetActive(!nodeReference.isObstacle &&
                                               nodeReference.jpDistances[(int)eDirections.SOUTH] > 0);
        southWestDistanceText.gameObject.SetActive(!nodeReference.isObstacle &&
                                                   nodeReference.jpDistances[(int)eDirections.SOUTH_WEST] > 0);
        westDistanceText.gameObject.SetActive(!nodeReference.isObstacle &&
                                              nodeReference.jpDistances[(int)eDirections.WEST] > 0);
        northWestDistanceText.gameObject.SetActive(!nodeReference.isObstacle &&
                                                   nodeReference.jpDistances[(int)eDirections.NORTH_WEST] > 0);
    }
    ...
    {
        northDistanceText.color = nodeReference.jpDistances[(int)eDirections.NORTH] > 0
            ? jumpPointDistanceColor
            : wallDistanceColor;
        northEastDistanceText.color = nodeReference.jpDistances[(int)eDirections.NORTH_EAST] > 0
            ? jumpPointDistanceColor
            : wallDistanceColor;
        eastDistanceText.color = nodeReference.jpDistances[(int)eDirections.EAST] > 0
            ? jumpPointDistanceColor
            : wallDistanceColor;
        southEastDistanceText.color = nodeReference.jpDistances[(int)eDirections.SOUTH_EAST] > 0
            ? jumpPointDistanceColor
            : wallDistanceColor;
        southDistanceText.color = nodeReference.jpDistances[(int)eDirections.SOUTH] > 0
            ? jumpPointDistanceColor
            : wallDistanceColor;
        southWestDistanceText.color = nodeReference.jpDistances[(int)eDirections.SOUTH_WEST] > 0
            ? jumpPointDistanceColor
            : wallDistanceColor;
        westDistanceText.color = nodeReference.jpDistances[(int)eDirections.WEST] > 0
            ? jumpPointDistanceColor
            : wallDistanceColor;
        northWestDistanceText.color = nodeReference.jpDistances[(int)eDirections.NORTH_WEST] > 0
            ? jumpPointDistanceColor
            : wallDistanceColor;
    }
        ...
        switch (JPSState.state)
        {
            case eJPSState.ST_OBSTACLE_BUILDING:
                GetComponent<SpriteRenderer>().sprite =
                    nodeReference.isObstacle == false ? passableSprite : obstacleSprite;
        ...
        switch (JPSState.state)
        {
            ...
            case eJPSState.ST_PLACE_SEARCH_ENDPOINTS:
                if (!nodeReference.isObstacle &&
                    !isPathEndPoint) // if we are not an obstacle, then let them select this node
                {
